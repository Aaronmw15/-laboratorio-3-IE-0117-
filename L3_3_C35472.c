...
int diagonal_1;
int diagonal_2;
for (int tipo=1;tipo<=2;tipo++){//este for realiza el recorrido primero para tipoA y luego para tipoB
    int secuencia=0;//esta lleva la cuenta de unos
    int secuencia_max=0;//esta guarda la cuenta más grande registrada

    int minimo=min(celdas[tipo], contador_celdas);//para ambos recorridos, encontramos el menor y el mayor para conocer el orden para recorrerlos
    int maximo=max(celdas[tipo], contador_celdas);//esto funciona porque los numeros de diagonal terminan siguiendo una secuencia creciente para ambos casos
    ...
    for (int diagonal=minimo; diagonal<=maximo; diagonal++){//en orden de numero de diagonal, evaluamos si el valor guardado es uno o cero
        for (int indice=0;indice<contador_celdas;indice++){//recorremos la matriz celdas
            if (celdas[tipo][indice]==diagonal){ //si encontramos un elemento con el numero de tipo que estamos buscando lo evaluamos
    ...
    if (tipo==1){
        diagonal_1=secuencia_max;
    }
    else{
        diagonal_2=secuencia_max;
    }
}   
    int diagonal_mas_larga=0;//comparamos entre las dos cuentas, para encontrar la mayor
    if (diagonal_1>=diagonal_2){
        diagonal_mas_larga=diagonal_1;
    }
    else{
        diagonal_mas_larga=diagonal_2;
    }
    return diagonal_mas_larga;
}
